...
import {
  ...
  dxUpdateUserNewUserRoleSelect as dxUpdateUserNewUserRoleSelectAction,
  ...
  dxUpdateEditUserRoleSelect as dxUpdateEditUserRoleSelectAction,
  ...
  dxUpdateUserSelectedUsersRoleSelect as dxUpdateUserSelectedUsersRoleSelectAction,
...
import InviteModal from '../presentation/InviteModal';
import UserModal from '../presentation/UserModal';
import UserPasswordModal from '../presentation/UserPasswordModal';
import UserDeleteModal from '../presentation/UserDeleteModal';
import UserGrantAccessModal from '../presentation/UserGrantAccessModal';
import UserLockModal from '../presentation/UserLockModal';
...
class MemberContainer extends Component {
  ...
  handleModalCreateUser = () => {
    ...
    if (!NewUserSelectedOptions.length) {
      this.props.dxAlertAction(true, true, 'Please select at least one role');
      return;
    }
    ...
    const assignRoles = [];
    NewUserSelectedOptions.forEach(role => {
      switch (role.value) {
        case 'ADMIN':
          assignRoles.push('keycloakOrgAdmin');
          break;
        case 'CONTENT':
          assignRoles.push('keycloakContentAdmin');
          break;
        case 'PUBLISH':
          assignRoles.push('keycloakPublishAdmin');
          assignRoles.push('keycloakChannelAdmin');
          break;
        case 'USER_MANAGE':
          assignRoles.push('keycloakUserManageAdmin');
          break;
        case 'LANGUAGE':
          assignRoles.push('keycloakLanguageAdmin');
          break;
        case 'ANALYTICS':
          assignRoles.push('keycloakAnalyticsAdmin');
          break;
      }
    });

    this.props.dxUpdateUserNewUserCreateAction(formattedUsers, assignRoles);
  };
  ...
  handleUpdateUserConfirm = () => {
    ...
    User.Roles = EditUserSelectedOptions;
  ...
  handleUpdateUserConfirmGrantAccess = () => {
    ...
    if (!SelectedUsersSelectedOptions.length) {
      this.props.dxAlertAction(true, true, 'At least one role to be selected');
      return;
    }
  ...
  handleUpdateUserConfirmRemoveAccess = () => {
    ...
    let tmpSelectedUsersSelectedOptions = [];
    SelectedUsersSelectedOptions.forEach(role => {
      switch (role) {
        case 'keycloakOrgAdmin':
          tmpSelectedUsersSelectedOptions.push('org-admin');
          break;
        case 'keycloakContentAdmin':
          tmpSelectedUsersSelectedOptions.push('content-admin');
          break;
        case 'keycloakPublishAdmin':
          tmpSelectedUsersSelectedOptions.push('publish-admin');
          break;
        case 'keycloakChannelAdmin':
          tmpSelectedUsersSelectedOptions.push('channel-admin');
          break;
        case 'keycloakUserManageAdmin':
          tmpSelectedUsersSelectedOptions.push('user-manage-admin');
          break;
        case 'keycloakLanguageAdmin':
          tmpSelectedUsersSelectedOptions.push('language-admin');
          break;
        case 'keycloakAnalyticsAdmin':
          tmpSelectedUsersSelectedOptions.push('analytics-admin');
          break;
      }
    });
    ...
    SelectedUsers.forEach((user, index) => {
      if (user.Roles) {
        let check = [];
        let tempRoles = user.Roles.split(',');
        for (let i = 0; i < tempRoles.length; i++) {
          let existedRole = tempRoles[i];
          for (let j = 0; j < tmpSelectedUsersSelectedOptions.length; j++) {
            let removeRole = tmpSelectedUsersSelectedOptions[j];
            if (existedRole == removeRole) {
              check.push(i);
            }
          }
        }
        if (check.length == tempRoles.length) {
          isValidate = false;
        }
      }
      if (!isValidate) {
        msg = `#${index +
          1}: Fail to remove role, user must have at least one role`;
      }
    });
    if (!isValidate) {
      this.props.dxAlertAction(true, true, msg);
      ...
      <div style={mainContainerStyle}>
        ...
        {!IsSelectMode ? (
          ...
          >
            ...
            >
              <p style={optionWrapperStyle}>assign roles</p>
            </div>
            <div
              style={optionContainerStyle}
              onClick={() => this.props.dxToggleRemoveAccessModalAction(true)}
            >
              <p style={optionWrapperStyle}>remove roles</p>
            </div>
            <div
              style={optionContainerStyle}
              onClick={() =>
        ...
        {IsUserInviteModalOpen && (
          <InviteModal
            ...
            handleSelectOption={options =>
              this.props.dxUpdateUserNewUserRoleSelectAction(options)
            }
        ...
        {IsUserModalOpen && (
          <UserModal
            ...
            handleSelectOption={options =>
              this.props.dxUpdateEditUserRoleSelectAction(options)
            }
        ...
        {IsGrantAccessModalOpen && (
          <UserGrantAccessModal
            ...
            handleSelectOption={options =>
              this.props.dxUpdateUserSelectedUsersRoleSelectAction(options)
            }
...
const stateToProps = state => {
  return {
    IsSelectMode: state.member.IsSelectMode,
    UserTotalRecords: state.member.UserTotalRecords,
    Users: state.member.Users,
    UserRoles: state.member.UserRoles,
    UserSelectedOptions: state.member.UserSelectedOptions,
    UserPageLimit: state.member.UserPageLimit,
    UserPageNumber: state.member.UserPageNumber,
    UserSearchValue: state.member.UserSearchValue,

    IsFetching: state.member.IsFetching,
    IsUserInviteModalOpen: state.member.IsUserInviteModalOpen,
    NewUsers: state.member.NewUsers,
    NewUserSelectedOptions: state.member.NewUserSelectedOptions,

    IsUserModalOpen: state.member.IsUserModalOpen,
    User: state.member.User,
    EditUserSelectedOptions: state.member.EditUserSelectedOptions,

    IsUserPasswordModalOpen: state.member.IsUserPasswordModalOpen,
    PasswordUserGUID: state.member.PasswordUserGUID,

    IsUserDeleteModalOpen: state.member.IsUserDeleteModalOpen,
    DeleteUserGUID: state.member.DeleteUserGUID,
    SelectedUsers: state.member.SelectedUsers,

    IsGrantAccessModalOpen: state.member.IsGrantAccessModalOpen,
    SelectedUsersSelectedOptions: state.member.SelectedUsersSelectedOptions,

    IsRemoveAccessModalOpen: state.member.IsRemoveAccessModalOpen,

    IsUserLockModalOpen: state.member.IsUserLockModalOpen,
    UserLockModalType: state.member.UserLockModalType,

    IsUsersDeleteModalOpen: state.member.IsUsersDeleteModalOpen
  };
};
...
const dispatchToProps = {
  ...
  dxUpdateUserSelectedUsersRoleSelectAction,
